...
/*   Updated: 2025/04/03 15:06:38 by nal-haki         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	get_cords_color(t_texture *img, int x, int y)
{
	...
	dst = img->pixel_bits + (y * img->length + x * (img->pixel_bits / 8));
	return (*(int *)dst);
}
...
void	initialize_textures(t_maindata *data, t_config *parsed_data)
{
	...
	while (i < 4)
	{
		...
		data->wall_textures[i].pixel_bits = mlx_get_data_addr
...
void	get_texture_color(t_maindata *data, t_ray *ray, int current_y)
{
	...
	ray->pixel_color = get_cords_color(&data->wall_textures[ray->texture_index],
