...
/*   Updated: 2025/03/24 17:28:36 by nal-haki         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	get_textures(char **words, t_config *scene_data, char *line)
{
	...
	error_handler("unknown element", words, line, scene_data);
}
...
void	line_parser(char *line, t_config *scene_data, int *map_flag)
{
	...
	words = ft_split(line, ' ');
	if (!words)
		error_handler("insufficient memory", NULL, line, scene_data);
	if (strings_len(words) != 2)
		error_handler("scene file syntax error\neach element \
must be followed by the exact information", words, line, scene_data);
...
void	file_parser(t_config *scene_data, char *scene_description_file)
{
	...
	if (!map_flag)
		error_handler("there no map in the file", NULL, line, scene_data);
